...
if __name__ == "__main__":
    ...
    trans_match_whole_df = pd.read_csv("transitions_match_results_whole.csv")
    trans_match_whole_df['Model'] = trans_match_whole_df['Model'].replace(model_name_mapping)
    protocol_dfs = {protocol: group.reset_index(drop=True) for protocol, group in trans_match_whole_df.groupby("Protocol")}
    print(trans_match_whole_df.head())
    #Generate LaTeX code for each protocol
    output_path = "all_transition_whole_metrics.tex"
    ...
    with open(output_path, "w") as out_f:
        ...
        for protocol, df in protocol_dfs.items():
            ...
            latex_code = df[
                ...
                column_format="llcccccc",
                longtable=False,
                caption=f"{protocol} Exact Transition Match Metrics",
                label=f"tab:{protocol.lower()}-whole-transition-matching-metrics",
                escape=False
            )
    ...
    '''Draw one concludeion table'''
    # Group by Model and calculate the sum for Total Extracted, Total GT, and Matched
    model_summary = trans_match_whole_df.groupby("Model")[["TotalExtracted", "TotalGT", "Matched"]].sum().reset_index()
    ...
    latex_code = model_summary.to_latex(
        ...
        column_format="lcccccc",
        longtable=False,
        caption="Overall Model Performance on Exact Transition Matching of Different Protocols",
        label="tab:whole-transition-match-summary",
        escape=False
    )
